...
def demo_grading_viz(estimate_next_pos_fcn, target_bot, OTHER = None):
    ...
    while not localized and ctr <= 10:
        ...
        if ctr == 10:
            print("Sorry, it took you too many steps to localize the target.")
        #More Visualization
        measured_broken_robot.setheading(target_bot.heading*180/np.pi)
        measured_broken_robot.goto(measurement[0]*size_multiplier, measurement[1]*size_multiplier-200)
        measured_broken_robot.stamp()
        broken_robot.setheading(target_bot.heading*180/np.pi)
        broken_robot.goto(target_bot.x*size_multiplier, target_bot.y*size_multiplier-200)
        broken_robot.stamp()
        prediction.setheading(target_bot.heading*180/np.pi)
...
if __name__ == "__main__":
    ...
    test_target = robot(2.1, 4.3, 0.5, 2*np.pi / 34.0, 1.5)
