# ------------
# 
# SLAM takes five parameters as input and returns the vector mu. This vector
# will have x, y coordinates interlaced, so for example, 
...
#                [Ly1]])
#
# data - This is the data that is generated with the
...
#                strength for motion is 1.0 / motion_noise.
#
# measurement_noise - The noise associated with measurement.
#                     The update strength for measurement is
...
def SLAM(data, N, num_landmarks, motion_noise, measurement_noise):
    Omega = matrix()
    Omega.zero(2 * (N + num_landmarks)) # * 2 for x and y measurements of each robot pose
    Omega.value[0][0] = 1
    Omega.value[1][1] = 1
    xi = matrix()
    xi.zero(2 * (N + num_landmarks), 1)
    xi.value[0][0] = world_size / 2     # robot starts in the middle of the world
    xi.value[1][0] = world_size / 2
    
    # for every motion step of the robot
    for i in range(len(data)):
        # get the index of the robot pose in Omega/xi
        n = i*2
        
        measurements = data[i][0]
        motion =       data[i][1]
        
        # update graph with measurements to seen landmarks
        for j in range(len(measurements)):
            
            # get the index of the landmark in Omega/xi
            m = 2 * (N + measurements[j][0])
            
            # update Omage/xi with the landmark measurements
            for k in range(2):
                ...
                xi.value[n+k][0]      -= measurements[j][k+1] / measurement_noise
                xi.value[m+k][0]      += measurements[j][k+1] / measurement_noise
                
        # update Omega/xi for the robot motion
        for k in range(4):
            Omega.value[n+k][n+k] += 1.0 / motion_noise
        for k in range(2):
            Omega.value[n+k  ][n+k+2] -= 1.0 / motion_noise
            Omega.value[n+k+2][n+k  ] -= 1.0 / motion_noise
            
            xi.value[n+k  ][0] -= motion[k] / motion_noise
            xi.value[n+k+2][0] += motion[k] / motion_noise
        
        
    # Compute estimate vector
    mu = Omega.inverse() * xi
...
# --------------------------------
#
# slam - retains only the latest robot pose and all landmarks
#
def online_SLAM(data, N, num_landmarks, motion_noise, measurement_noise):
    output_dim = 2 * (1 + num_landmarks)
    
    Omega = matrix()
    Omega.zero(output_dim) # * 2 for x and y measurements of each robot pose
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    xi = matrix()
    xi.zero(output_dim, 1)
    xi.value[0][0] = world_size / 2.0     # robot starts in the middle of the world
    ...
    for i in range(len(data)):
        ...
        for j in range(len(measurements)):
            ...
            for k in range(2):
                ...
                xi.value[k][0]      -= measurements[j][k+1] / measurement_noise
                xi.value[m+k][0]      += measurements[j][k+1] / measurement_noise
                
                
        # expand the information matrix and vector to make room for a new robot pose
        expansion = [0,1] + list(range(4, output_dim+2)) 
        Omega = Omega.expand(output_dim+2, output_dim+2, expansion, expansion)
        xi = xi.expand(output_dim+2, 1, expansion, [0])
                
                
        # update Omega/xi for the robot motion
        for k in range(4):
            Omega.value[k][k] += 1.0 / motion_noise
        for k in range(2):
            Omega.value[k][k+2] -= 1.0 / motion_noise
            Omega.value[k+2][k] -= 1.0 / motion_noise
            
            xi.value[k  ][0] -= motion[k] / motion_noise
            xi.value[k+2][0] += motion[k] / motion_noise
            
        # factor out the previous pose with some matrix magic
        contraction = range(2, len(Omega.value))
        A = Omega.take([0,1], contraction)
        B = Omega.take([0,1])
        C = xi.take([0, 1], [0])
        
        Omega = Omega.take(contraction) - A.transpose() * B.inverse() * A
        xi = xi.take(contraction, [0]) - A.transpose() * B.inverse() * C
...
if __name__ == "__main__":
    ...
    print_result(1, num_landmarks, result)
