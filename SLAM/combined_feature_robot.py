    ...
    def get_position(self):
        ...
        for i in range(self.N):
            ...
            orientation += (((self.data[i].orientation
                              - self.data[0].orientation + np.pi) % (2.0 * np.pi)) 
                            + self.data[0].orientation - np.pi)
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # Projection of robot onto path segment
        u = (Rx*dx + Ry*dy) / np.sqrt(dx**2 + dy**2)
        # if we are past the end of the current path segment, move to the next one
        if u > 1.0:
            index += 1
            
            # recalculate all our position vectors
        ...
        cte = (Ry*dx - Rx*dy) / np.sqrt(dx**2 + dy**2)
